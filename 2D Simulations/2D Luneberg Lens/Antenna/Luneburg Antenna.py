...
def draw_rect(material,espr,width,height,x_pos,y_pos):
    if material == 'vacuum':
        ...
        for x in range(0, height):
            for y in range(0, width):
                C1[x+y_pos, y+x_pos] = 1 / (epsr + (1e14 * dt / eps_0))
                C2[x+y_pos, y+x_pos] = (1e14 * dt / eps_0)
        ...
        for i in range(a, b):
            ...
            if dist <= radius:
                if material == 'vacuum':
                    ...
                    C1[i, j] = 1/(epsr + (1e14*dt/eps_0))
                    C2[i, j] = (1e14*dt/eps_0)
                   

# DIELECTRIC CONFIGURATIONS
def luneberg(R):
    for r in range(R,0,-1):
        draw_circ('dielectric',2-(r/R)**2,r,150,150)
...
draw_rect('vacuum',1,500,300,0,0)
draw_circ('metal',4,102,150,150)
draw_rect('vacuum',1,150,300,0,0)
draw_rect('vacuum',1,150,75,100,0)
draw_rect('vacuum',1,150,75,100,225)
luneberg(100)
draw_circ('metal',4,60,380,105)
draw_circ('vacuum',4,58,380,105)
draw_circ('metal',4,30,382,107)
draw_circ('vacuum',4,28,382,107)
draw_rect('metal',1,130,30,250,135) #Left arm
draw_rect('vacuum',1,130,26,250,137) #Left arm
draw_rect('metal',1,30,105,410,0)#right arm
draw_rect('vacuum',1,26,105,412,0)  #right arm
draw_rect('vacuum',1,90,135,290,0) #Cleanup
draw_rect('vacuum',1,30,100,380,0)
